...
public:
    ...
    template <size_t N = 9>
    std::unordered_map<uint64_t, BlockArrayValue<N>> extractResponse(vector<std::vector<std::vector<unsigned char>>> entries_list, vector<uint64_t> cuckoo_table);
...
template <size_t N = 9>
BlockArrayValue<N> convertVectorToBlockArrayValue(const std::vector<unsigned char> &byteVector)
{
    size_t byteVectorSize = byteVector.size();

    BlockArrayValue<N> result;

    std::memcpy(result.var, byteVector.data(), byteVectorSize);
...
std::unordered_map<uint64_t, BlockArrayValue<N>> BatchPIRClient::extractResponse(vector<std::vector<std::vector<unsigned char>>> entries_list, vector<uint64_t> cuckoo_table)
{
    ...
    size_t num_servers = ceil(num_buckets / per_server_capacity);
    auto previous_idx = 0;
    std::unordered_map<uint64_t, BlockArrayValue<N>> extract_ans;
    ...
    for (int i = 0; i < entries_list.size(); i++)
    {
        ...
        for (auto j = 0; j < entries_list[i].size(); j++)
        {
            ...
            {
                extract_ans[sub_buckets[j]] = (convertVectorToBlockArrayValue<N>(entries_list[i][j]));
            }
        }
        previous_idx += offset;
    }
